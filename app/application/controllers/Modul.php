...
class Modul extends CI_Controller {
	...
	function get_airport(){
		...
		$airport = $this->Modul_Model->execqueryreturn("select * from airport");
	...
	function book_flight(){
		...
		$this->writeFile("result_book_flight_".$token.".txt", $result);
		
		$resultx = json_decode($result, true);
		if(isset($resultx["diagnostic"]["confirm"])){
	...
	function sync_book_data($token, $c){
		...
		$this->writeFile("sync_flight_".$token.".txt", $bookFlight);
		...
		if(isset($bookFlight["myorder"])){
			...
			foreach($bookFlightMyOrder as $data){
				...
				if($order_type == "flight"){
					...
					$this->writeFile("q_".$token.".txt", $query);
					...
					$this->writeFile("req_".$token.".txt", $query);
	...
	function get_history_booking(){
		...
		$q = "select order_id, 'flight' as type from order_flight where email = '".$email."' and status <> -1 union
				select order_id, 'hotel' as type from order_hotel where email = '".$email."' and status <> -1  
